...
- [X] T009a Dev-only M0 measurement probe harness (M0 support — NOT the Intent pipeline)
  - Metadata: Milestone=M0; Title=M0 probe harness; Description=Isolated dev/POC-only harness under `systems/project_management/lib/features/ai_assistant/m0_probe/` to run T007/T008 on the S22 Ultra: a fixed prompt template per variant (`assets/ai/prompts/m0_strict_json_depth_2.txt`, `..._depth_1.txt`) with a `{{USER_QUESTION}}` placeholder; `M0ProbeController` loads the template, injects the typed question, and calls the existing `LocalSlmService.generateText` (reused, not reimplemented), measuring latency; `M0ProbeView` dev screen (question input, depth-2/depth-1 selector, Run, raw-output panel, latency, copy, manual Yes/No/— checks, notes); `[AI_M0_PROBE]` debug-only logs; reached only behind the `kM0ProbeEnabled` (`--dart-define=AI_M0_PROBE=true`) flag via an additive, flag-gated launcher in the AI Assistant model-selection branch; Files touched=`lib/features/ai_assistant/m0_probe/m0_probe_flags.dart`,`.../m0_probe_log.dart`,`.../m0_probe_controller.dart`,`.../m0_probe_view.dart`,`assets/ai/prompts/m0_strict_json_depth_2.txt`,`assets/ai/prompts/m0_strict_json_depth_1.txt`,`pubspec.yaml`,`lib/core/di/project_management_locator.dart`,`lib/features/ai_assistant/view/ai_assistant_view.dart`; Dependencies=T003; Acceptance check=tester types only the question; prompt is fixed internally and injected into `{{USER_QUESTION}}`; existing `LocalSlmService` reused; flag off → no change to 001 chat behavior; NO SchemaGraph/loader/trimmer/parser/validator/repair/IntentService/production Intent Debug UI; does NOT mark G-M0 passed; Execution=automated (harness) + manual (device run); CI=CI-safe (no real inference in CI).
  - Note: This is **M0 measurement support only**. It explicitly does NOT count as M2+ Intent pipeline implementation, and the `intent/` pipeline folder is still not created. G-M0 remains pending until real S22 Ultra results (T007/T008) are recorded and reviewed.
