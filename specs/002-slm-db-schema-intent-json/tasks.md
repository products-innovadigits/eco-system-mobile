...
- [ ] T009b Execute the M0 measurement checklist on Samsung Galaxy S22 Ultra (planning/measurement — no production code)
  - Metadata: Milestone=M0; Title=Run M0 measurement checklist; Description=Follow `specs/002-slm-db-schema-intent-json/m0_probe/m0_measurement_checklist.md`: capacity runs (depth-0@1024, depth-0@1280, depth-1@1280, depth-2 guard-block), exact-token-counting investigation, decoding-parameter (temp/topK/topP) investigation, first-vs-second-run latency, and the output-quality scores (valid JSON, schema grounding, invented value, relationship correctness, risk-vs-priority confusion, clarification behavior) plus crash/memory/heat notes; record results in `result_recording_template.md`; Files touched=`specs/002-slm-db-schema-intent-json/m0_probe/*` (records only); Dependencies=T009a; Acceptance check=checklist filled with measured numbers against the G-M0 decision-criteria table; does NOT flip G-M0; Execution=manual (device); CI=manual-only.
- [ ] T009c Run the golden set + produce the M0 scorecard (measurement — no production code)
  - Metadata: Milestone=M0; Title=Golden-set run + scorecard; Description=Run `m0_golden_questions.md` (50 questions, AR/EN/mixed across risk/priority/delayed/manager/department/lifecycle/budget/count/list/compare/summary/ambiguous/unsupported) through the probe at the best-fitting config; record raw outputs + per-category scores; produce the aggregate scorecard; Files touched=`specs/002-slm-db-schema-intent-json/m0_probe/*`; Dependencies=T009b; Acceptance check=>=50 questions scored, per-category + aggregate scorecard produced; `research.md` NOT updated with final conclusions; G-M0 NOT flipped; Execution=manual (device); CI=manual-only.
- [X] T009d Deterministic-grounding spike (dev-only / off-device — NOT production)
  - Metadata: Milestone=M0; Title=Deterministic-grounding spike; Description=Per `deterministic_grounding_spike_scope.md`, prototype off-device: normalizer, business-vocabulary lexicon, lookup-value matcher, schema/FK graph matcher, candidate ranker, validator; compare raw-SLM vs deterministic vs hybrid on the golden set (schema-grounded %, invented-value %, risk-vs-priority confusion, relationship %, clarification %, prompt-size reduction); Files touched=dev-only spike harness (NOT in the Flutter app) + a findings note under `m0_probe/`; Dependencies=T009c; Acceptance check=three-arm comparison + recommendation on how much deterministic grounding closes the gap and what the SLM should still own; NO production Schema Router/Slicer/Validator/Repair/IntentService, no `intent/` folder, no SQL/DB/retrieval; G-M0 NOT flipped; Execution=manual/dev; CI=n/a (off-device).

**Checkpoint M0**: Stop if T009 does not record G-M0 as passed. M1 documentation and asset cleanup may continue, but no M2+ on-device pipeline implementation starts or merges. (T009a, the dev-only measurement harness, and T009b–T009d, the measurement/spike planning tasks, are permitted M0 support and are NOT M2+.)
