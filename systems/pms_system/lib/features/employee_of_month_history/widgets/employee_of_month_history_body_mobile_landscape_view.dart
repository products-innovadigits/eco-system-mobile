...
class EmployeeOfMonthHistoryBodyMobileLandscapeView extends StatelessWidget {
  final ScrollController scrollController;
  final TextEditingController searchController;

  const EmployeeOfMonthHistoryBodyMobileLandscapeView({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: BlocBuilder<EmployeeOfMonthHistoryBloc,
          EmployeeOfMonthHistoryState>(
        builder: (context, state) {
          return switch (state) {
            EmployeeOfMonthHistoryInitial() =>
              ShimmerCardsList(cardHeight: 200),
            EmployeeOfMonthHistoryLoading() =>
              ShimmerCardsList(cardHeight: 200),
            EmployeeOfMonthHistoryLoaded(:final items, :final isLoadingMore) =>
              Column(
                children: [
                  Expanded(
                    child: ListAnimator(
                      ...
                      data: items
                          .map(
                            (e) => EmployeeOfMonthHistoryCard(item: e),
                          )
                          .toList(),
                    ),
                  ),
            ...
            EmployeeOfMonthHistoryEmpty(:final isInitial) => _HandleEmptyList(
                initial: isInitial,
