  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
          ...
          withSearch: true,
          onTapSearch: () => CustomNavigator.push(Routes.SEARCH , arguments: SearchEnum.jobs),
          searchHintText:
              allTranslations.text(LocaleKeys.searching_for_job)),
      body: Padding(
        padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 16.h),
        child: BlocBuilder<JobsBloc, AppState>(
          builder: (context, state) {
            if (state is Loading) return LoadingShimmerList();
            if (state is Done) return Column(
              children: [
